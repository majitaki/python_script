...
for data_kind in ['normal', 'norm']:
    for eva_kind in ['ave', 'std']:
        ...
        ax2 = df_data['ccvTP'].plot(kind='bar', color='orange', ax=ax2, width=width, position = 0)
        ...
        if data_kind == 'norm':
            ...
            if eva_kind == 'ave':
                ...
                ax2.set_yticks(np.linspace(0, ccvtp_ave_thd, 6)) 
                ax1.set_ylim([0, lfhf_ave_thd])
                ax2.set_ylim([0, ccvtp_ave_thd])
            elif eva_kind == 'std':
                ax1.set_yticks(np.linspace(0, lfhf_std_thd, 6)) 
                ax2.set_yticks(np.linspace(0, ccvtp_std_thd, 6)) 
                ax1.set_ylim([0, lfhf_std_thd])
                ax2.set_ylim([0, ccvtp_std_thd])
                    
            
        
        ax1.set_title(sj_name + '_' + data_kind + '_' + eva_kind + '_LFHF_and_ccvTP' , fontsize = 15)
        ax1.legend(loc='upper left', fontsize = 15)
        ax2.legend(loc='upper right', fontsize = 15)
        ax1.set_xlabel("time", fontsize = 15)
        
        ax.set_ylabel('LF/HF', fontsize = 15)
        ax2.set_ylabel('ccvTP', fontsize = 15)
        ...
        output_image_path = output_folder + sj_name + '_stress_lfhf_ccvtp' + '_' + data_kind + '_' + eva_kind + '.png'
