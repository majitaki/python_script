...
for index, clm in  tqdm(zip(range(0, len(graph_list)), graph_list)):
    res = sm.tsa.seasonal_decompose(trim_data[clm], freq = res_freq, model='multiplicative')
    
    ax1 = res.seasonal.plot(marker = 'o', label = clm, fontsize=15)
...
cv2.imwrite(output_folder + 'output_compare_all_norm_seasonal_' + sj_name + '.png', united_imgs)