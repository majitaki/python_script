...
graph_list = ['HR', 'ccvTP', 'LF', 'HF', 'LF_HF', 'LF+HF']
imgs = []

for index, clm in  tqdm(zip(range(0, len(graph_list)), graph_list)):
    ax1 = trim_data.temp.plot(marker = 'o', label = 'temp', fontsize=15)
    ax2 = ax1.twinx()
    ax2 = trim_data[clm].plot(marker = 'o', label = clm, fontsize=15, style = ['r--', 'b.-'])
    
    ax1.set_xlabel("time", fontsize = 15)
    ax1.set_ylabel(r"temp", fontsize = 15)
    ax2.set_ylabel(clm, fontsize = 15)
    
    ax1.set_title(sj_name + '_' + 'temp' + '_vs_' + clm , fontsize = 15)
    ax1.legend(loc='upper left', fontsize = 15)
    ax2.legend(loc='upper right', fontsize = 15)
    
    ax1_y_max = round(ax1.get_ybound()[1]) + 1
    ax1_y_min = round(ax1.get_ybound()[0])
    ax2_y_max = round(ax2.get_ybound()[1]) + 1
    ...
    ax2.set_yticks(np.linspace(ax2_y_min, ax2_y_max, 6)) 
...
cv2.imwrite(output_folder + 'output_compare_all_' + sj_name + '.png', united_imgs)