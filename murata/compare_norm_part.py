...
graph_list = ['temp', 'ccvTP', 'LF', 'HF', 'HF_Norm', 'LF+HF']
imgs = []

for index, clm in  tqdm(zip(range(0, len(graph_list)), graph_list)):
    res = sm.tsa.seasonal_decompose(trim_data[clm], freq=7)
    
    ax1 = trim_data[clm].plot(marker = 'o', label = clm, fontsize=15)
    ax2 = ax1.twinx()
    ax2 = res.trend.plot(label = clm, fontsize=15, style = ['r--', 'b.-'])
    ...
    ax2.set_yticks(np.linspace(ax1_y_min, ax1_y_max, 6)) 
    ax1.set_ylim([ax1_y_min, ax1_y_max])
    ax2.set_ylim([ax1_y_min, ax1_y_max])
...
cv2.imwrite(output_folder + 'output_compare_all_norm_' + sj_name + '.png', united_imgs)