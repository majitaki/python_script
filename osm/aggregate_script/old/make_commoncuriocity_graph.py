...
for mydir in dir_list:
    ...
    commoncuriocity = mydir.split('_')[9]
    commoncuriocity = round(float(commoncuriocity),2)
    max_curiocities.append(commoncuriocity)
    
    
network_list=list(set(network_list))
max_curiocities=list(set(max_curiocities))
max_curiocities.sort()

df_target = pd.DataFrame(index = max_curiocities, columns=network_list)
df_incorrect_target = pd.DataFrame(index = max_curiocities, columns=network_list)
df_undeter_target = pd.DataFrame(index = max_curiocities, columns=network_list)
df_cor_incor_target = pd.DataFrame(index = max_curiocities, columns=network_list)
df_better_simpsond = pd.DataFrame(index = max_curiocities, columns=network_list)
...
for network in network_list:
    ...
    df_target[network] = pd.DataFrame(accs, index = max_curiocities[:len(accs)], columns = [network])
    df_incorrect_target[network] = pd.DataFrame(none_accs, index = max_curiocities[:len(none_accs)], columns = [network])
    df_undeter_target[network] = pd.DataFrame(undeter_accs, index = max_curiocities[:len(undeter_accs)], columns = [network])
    df_cor_incor_target[network] = pd.DataFrame(cor_incor_accs, index = max_curiocities[:len(cor_incor_accs)], columns = [network])
    df_better_simpsond[network] = pd.DataFrame(better_simpsond, index = max_curiocities[:len(better_simpsond)], columns = [network])
    
df_target.plot(grid = True, marker = 'o', figsize=(10,6))
plt.title('cor_curiocity_' + algo + '_dim' + str(dim))
plt.xticks(np.arange(min(max_curiocities), max(max_curiocities) + 0.1, 0.1 ))
plt.yticks(np.arange(0.0, 1.05, 0.1))
plt.xlabel("common_curiocity, cc", fontsize=18)
plt.ylabel("Accuracy, R", fontsize=18)
plt.tick_params(labelsize=20)
plt.savefig('commoncuriocity_correct_' +  str(dim) + '.png')
#plt.close('all')

df_incorrect_target.plot(grid = True, marker = 'o', figsize=(10,6))
plt.title('incor_curiocity_' + algo + '_dim' + str(dim))
plt.xticks(np.arange(min(max_curiocities), max(max_curiocities) + 0.1, 0.1 ))
plt.yticks(np.arange(0.0, 1.05, 0.1))
plt.xlabel("common_curiocity, cc", fontsize=18)
plt.ylabel("Not Accuracy, Not R", fontsize=18)
plt.tick_params(labelsize=20)
plt.savefig('commoncuriocity_incorrect_' +  str(dim) + '.png')

df_undeter_target.plot(grid = True, marker = 'o', figsize=(10,6))
plt.title('undeter_curiocity_' + algo + '_dim' + str(dim))
plt.xticks(np.arange(min(max_curiocities), max(max_curiocities) + 0.1, 0.1 ))
plt.yticks(np.arange(0.0, 1.05, 0.1))
plt.xlabel("common_curiocity, cc", fontsize=18)
plt.ylabel("Undeter Rate", fontsize=18)
plt.tick_params(labelsize=20)
plt.savefig('commoncuriocity_undeter_' +  str(dim) + '.png')

df_cor_incor_target.plot(grid = True, marker = 'o', figsize=(10,6))
plt.title('balance_acc_curiocity_' + algo + '_dim' + str(dim))
plt.xticks(np.arange(min(max_curiocities), max(max_curiocities) + 0.1, 0.1 ))
plt.yticks(np.arange(0.0, 4.4, 0.5))
plt.xlabel("common_curiocity, cc", fontsize=18)
plt.ylabel("Balance Accuracy", fontsize=18)
plt.tick_params(labelsize=20)
plt.savefig('commoncuriocity_balance_' +  str(dim) + '.png')
...
plt.tick_params(labelsize=20)
plt.savefig('commoncuriocity_better_simpsond_' +  str(dim) + '.png')